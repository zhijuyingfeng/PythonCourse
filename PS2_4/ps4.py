...
def simulation_without_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        ...
        population.append(bacteria_num[:])
    avgs=[]
    for i in range(301):
        avgs.append(calc_pop_avg(population,i))
    x_coords=list(range(301))
    make_one_curve_plot(x_coords,avgs,"TimeStamp","Average Population","Without Antibiotic")
    return population
...
def calc_pop_std(populations, t):
    ...
    std=0
    for i in range(num_trials):
        std+=pow(populations[i][t]-avg,2)
    return math.sqrt(std/num_trials)
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        pop_density=self.get_total_pop()/self.max_pop
        for bacteria in surviving_bacteria:
            child=ResistantBacteria(0,0,False,0)
            try:
                child=bacteria.reproduce(pop_density)
            except NoChildException:
                continue
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        bacteria=[]
        total_bacteria_num=[num_bacteria]
        resistant_bacteria_num=[]
        for j in range(num_bacteria):
        ...
        for j in range(250):
            total_bacteria_num.append(patient.update())
            resistant_bacteria_num.append(patient.get_resist_pop())
        total.append(total_bacteria_num[:])
        resistants.append(resistant_bacteria_num[:])

    total_avg=[]
    resistant_avg=[]
    x_coords=list(range(401))
    for i in range(401):
