...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        ans="%s->%s (%d, %d)"%(self.src.get_name(),self.dest.get_name(),self.total_distance,self.outdoor_distance)
        return ans
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        previous_number=len(self.nodes)
        self.nodes.add(node)
    ...
    def add_edge(self, edge):
        ...
        if (not edge.src in self.nodes) or (not edge.dest in self.nodes):
            raise ValueError("The nodes associated with the edge is not in the graph.")
        l=self.edges.get(edge.get_source(),[])
        l.append(edge)
        self.edges[edge.get_source()]=l
        # l=self.edges.get(edge.get_destination(),[])
        # l.append(edge)
        # self.edges[edge.get_destination()]=l
